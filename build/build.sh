...
if [ -d jstree/ ]; then
  $YUICOMPRESSOR --nomunge -o jstree/jquery.jstree.min.js jstree/jquery.jstree.js &
fi
...
ls -1 jquery.*.css font-awesome.css demo_table.css | grep -v .min.css | xargs cat > jquery_plugins_and_other.css
ls -1 jquery.*.css font-awesome.css demo_table.css | grep -v .min.css | xargs rm
